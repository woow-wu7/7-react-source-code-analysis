...
export type Hook = {|
  memoizedState: any, // 指向当前渲染节点Fiber，上一次完整更新后的最终状态值，用来记录当前 ( useState ) 应该 ( 返回的结果 )，缓存
  baseState: any, // 初始化 initialState, 以及每次 dispatch 之后 newState
  baseQueue: Update<any, any> | null,
  queue: UpdateQueue<any, any> | null, // 缓存队列，储存多次更新行为
  next: Hook | null, // 指向下一次 useState 对应的 Hook 对象
|};
  ...
  if (__DEV__) {
    ...
    ReactCurrentDispatcher.current =
      current === null || current.memoizedState === null
        ...
        : HooksDispatcherOnUpdate; // ----- update阶段dispatcher
  }

  let children = Component(props, secondArg); // 此时已经有了新的 dispatcher, 在调用 Component 时就可以拿到新的对象
